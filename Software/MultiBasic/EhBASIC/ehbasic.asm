...
#
|;   2023/05/21 - techav
|;       converted to GNU AS syntax & modified to run from memory on Wrap030-ATX
#
...
DELAY   SUBQ.l          #1,%d0
        ...
        MOVE.L          #0,%d0              |; Tells LIST no arguments
        ANDI.b          #0xFE,%CCR          |; Clear carry
        BSR             LAB_LIST                        |; Call LIST routine

        MOVE.L          #'~',%d0                        |; Send tilde symbol indicate end of file (used when loading)
        BSR             LAB_PRNA

        MOVE.L          #26,%d0             |; Send Control-Z to indicate end of file save operation
        BSR             LAB_PRNA
...
LAB_COLD:
    ...
    MOVE.L  #5,%d0                          |; error 5 - not enough RAM
...
LAB_FOER:
    MOVE.L  #0x2C,%d7                       |; error code 0x2C "Format" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_ADER:
    MOVE.L  #0x2A,%d7                       |; error code 0x2A "Address" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_WDER:
    MOVE.L  #0x28,%d7                       |; error code 0x28 "Wrong dimensions" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_UDER:
    MOVE.L  #0x26,%d7                       |; error code 0x26 "undimensioned array" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
|; value at the top of this file set to zero
    ...
    MOVE.L  #0x24,%d7                       |; error code 0x24 "undefined variable" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
|; value at the top of this file to some non zero value
    ...
    MOVE.L  #0,%d0                          |; clear %d0 and set the zero flag
...
LAB_LDER:
    MOVE.L  #0x22,%d7                       |; error code 0x22 "LOOP without DO" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_UFER:
    MOVE.L  #0x20,%d7                       |; error code 0x20 "Undefined function" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_CCER:
    MOVE.L  #0x1E,%d7                       |; error code 0x1E "Can't continue" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_SCER:
    MOVE.L  #0x1C,%d7                       |; error code 0x1C "String too complex" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_SLER:
    MOVE.L  #0x1A,%d7                       |; error code 0x1A "String too long" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_TMER:
    MOVE.L  #0x18,%d7                       |; error code 0x18 "Type mismatch" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_IDER:
    MOVE.L  #0x16,%d7                       |; error code 0x16 "Illegal direct" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_DZER:
    MOVE.L  #0x14,%d7                       |; error code 0x14 "Divide by zero" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_DDER:
    MOVE.L  #0x12,%d7                       |; error code 0x12 "Double dimension" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_ABER:
    MOVE.L  #0x10,%d7                       |; error code 0x10 "Array bounds" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_USER:
    MOVE.L  #0x0E,%d7                       |; error code 0x0E "Undefined statement" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_OMER:
    MOVE.L  #0x0C,%d7                       |; error code 0x0C "Out of memory" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_OFER:
    MOVE.L  #0x0A,%d7                       |; error code 0x0A "Overflow" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_FCER:
    MOVE.L  #0x08,%d7                       |; error code 0x08 "Function call" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_ODER:
    MOVE.L  #0x06,%d7                       |; error code 0x06 "Out of DATA" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_RGER:
    MOVE.L  #0x04,%d7                       |; error code 0x04 "RETURN without GOSUB" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_SNER:
    MOVE.L  #0x02,%d7                       |; error code 0x02 "Syntax" error
    BRA     LAB_XERR                        |; do error #%d7, then warm start
...
LAB_NFER:
    MOVE.L  #0x00,%d7                       |; error code 0x00 "NEXT without FOR" error
...
LAB_1269:
    ...
    BMI     LAB_1274                        |; go do warm start if -ve |; (was immediate mode)
...
LAB_1295:
    ...
    BCS     LAB_12E6                        |; branch if not found
...
|; space is opened up, now copy the crunched line from the input buffer into the space
    ...
    MOVEA.l %a0,%a2                         |; copy destination
    MOVE.L  #-1,%d1                         |; set to allow re-chaining
...
LAB_1325:
    ...
    BNE     LAB_132E                        |; not end of program yet so we must
...
LAB_134B:
    ...
    MOVE.L  #' ',%d0                        |; load [SPACE]
    BSR     LAB_PRNA                        |; go print
    MOVE.L  #0x08,%d0                       |; load [BACKSPACE]
    BSR     LAB_PRNA                        |; go print
    SUBQ.w  #0x01,%d1                       |; decrement the buffer index (delete)
    BRA     LAB_1359                        |; re-enter loop
...
LAB_1357:
    MOVE.L  #0x00,%d1                       |; clear buffer index
...
|; is empty
    ...
    BLS     LAB_1359                        |; if < ignore character

###    CMP.b        #' '+1,%d0              |; compare with [SP]+1
###    BCS          LAB_1359                |; if < ignore character
...
LAB_1378:
    ...
    BCC     LAB_138E                        |; skip store & do [BELL] if buffer full
...
LAB_138E:
    MOVE.L  #0x07,%d0                       |; [BELL] character into d0
...
LAB_1392:
    ...
    BEQ     LAB_1392                        |; if [SPACE] just go save it and get another

    CMP.b   #'0',%d0                        |; compare the character with "0"
    BCS     LAB_13C6                        |; if < "0" quit the hex save loop

    CMP.b   #'9',%d0                        |; compare with "9"
    BLS     LAB_1392                        |; if it is "0" to "9" save it and get another

    MOVE.L  #-33,%d5                        |; mask xx0x xxxx, ASCII upper case
    AND.b   %d0,%d5                         |; mask the character

    CMP.b   #'A',%d5                        |; compare with "A"
    BCS     LAB_13CC                        |; if < "A" quit the hex save loop

    CMP.b   #'F',%d5                        |; compare with "F"
    BLS     LAB_1392                        |; if it is "A" to "F" save it and get another

    BRA     LAB_13CC                        |; else continue crunching
...
LAB_13A6:
    MOVE.L  #0,%d1                          |; clear the read index
    MOVE.l  %d1,%d2                         |; clear the save index
    MOVE.b  %d1,Oquote(%a3)                 |; clear the open quote/DATA flag
LAB_13AC:
    MOVE.L  #0,%d0                          |; clear word
    MOVE.b  (%a5,%d1.w),%d0                 |; get byte from input buffer
    BEQ     LAB_13EC                        |; if null save byte then continue crunching

    CMP.b   #'_',%d0                        |; compare with "_"
    BCC     LAB_13EC                        |; if >= "_" save byte then continue crunching

    CMP.b   #'<',%d0                        |; compare with "<"
    BCC     LAB_13CC                        |; if >= "<" go crunch

    CMP.b   #'0',%d0                        |; compare with "0"
    BCC     LAB_13EC                        |; if >= "0" save byte then continue crunching
    ...
    BEQ     LAB_1392                        |; if so go copy a hex value

LAB_13C6:
    CMP.b   #'*',%d0                        |; compare with "*"
    BCS     LAB_13EC                        |; if <= "*" save byte then continue crunching

                                            |; crunch rest
LAB_13CC:
    BTST.b  #6,Oquote(%a3)                  |; test open quote/DATA token flag
    BNE     LAB_13EC                        |; branch if b6 of Oquote set (was DATA)
...
LAB_13EC:
    ...
    BNE     LAB_1401                        |; branch if not DATA
                                            ...
                                            |; token was : or DATA
LAB_13FF:
    MOVE.b  %d0,Oquote(%a3)                 |; save token-0x3A (0x00 for ":", TK_DATA-0x3A for
                                            |; DATA)
LAB_1401:
...
LAB_1408:
    ...
    BEQ     LAB_13EC                        |; branch if match (end quote, REM, :, or DATA)
...
LAB_141B:
    ...
    BPL     LAB_141B                        |; if not end of keyword go do next byte

    ADDQ.w  #1,%d3                          |; increment keyword table pointer flag
                                            |; unchanged
    MOVE.b  (%a1,%d3.w),%d0                 |; get keyword table byte
    BNE     LAB_13D8                        |; go test next word if not zero byte (table end)

|; reached end of table with no match
LAB_141F:
    MOVE.b  (%a5,%d1.w),%d0                 |; restore byte from input buffer
    BRA     LAB_13EC                        |; go save byte in output and continue crunching

|; reached [EOL]
LAB_142A:
    MOVE.L  #0,%d0                          |; ensure longword clear
    BTST    %d0,%d2                         |; test odd bit (fastest)
    BEQ     LAB_142C                        |; branch if no bytes to fill
...
LAB_SSLN:
    ...
    BRA     LAB_SCLN                        |; go search for required line from a0
...
LAB_SCLN:
    ...
    BEQ     LAB_145E                        |; is end marker so we're done, do 'no line' exit

    CMP.l   (%a0),%d1                       |; compare this line |; with required line #
    BGT     LAB_145F                        |; loop if required |; > this #
...
LAB_CLEAR:
    BEQ     LAB_147A                        |; if no following byte go do "CLEAR"
...
LAB_LIST:
    BCS     LAB_14BD                        |; branch if next character numeric (LIST n...)

    MOVE.L  #-1,%d1                         |; set end to 0xFFFFFFFF
    MOVE.l  %d1,Itemp(%a3)                  |; save to Itemp

    MOVE.L  #0,%d1                          |; set start to 0x00000000
    TST.b   %d0                             |; test next byte
    BEQ     LAB_14C0                        |; branch if next character [NULL] (LIST)

    CMP.b   #TK_MINUS,%d0                   |; compare with token for -
    BNE     RTS_005                         |; exit if not - (LIST -m)
...
|; this bit checks the - is present
    CMP.b   #TK_MINUS,%d0                   |; compare with token for -
    BNE     RTS_005                         |; return if not "-" (will be Syntax error)
...
|; there is an m to set as the end value
    ...
    BEQ     LAB_14D4                        |; branch if was [NULL] (LIST n-)
...
LAB_14D4:
    ...
    BHI     RTS_005                         |; if this line greater all done so exit

LAB_14E2:
    MOVEM.l %a0-%a1,-(%sp)                  |; save registers
    BSR     LAB_295E                        |; print %d0 as unsigned integer
    MOVEM.l (%sp)+,%a0-%a1                  |; restore registers
    MOVE.L  #0x20,%d0                       |; space is the next character
LAB_150C:
    BSR     LAB_PRNA                        |; go print the character
...
LAB_1519:
    ...
    BNE     LAB_152E                        |; branch if not [EOL] (go print)

                                            |; was [EOL]
    MOVEA.l %a1,%a0                         |; copy next line pointer
    MOVE.l  %a0,%d0                         |; copy to set flags
    BNE     LAB_14D4                        |; go do next line if not [EOT]
...
LAB_152E:
    ...
    BMI     LAB_1519                        |; if -ve done so go get next byte
...
|;    MOVE.L       #28,%d0                  |; we need 28 bytes !
|;    BSR          LAB_1212                 |; check room on stack for %d0 bytes
...
LAB_END:
    BNE     LAB_1639                        |; exit if something follows STOP
...
LAB_163B:
    ...
    BCS     LAB_164F                        |; branch if BASIC pointer is in buffer
...
LAB_RUNn:
    ...
    BRA     LAB_16B0                        |; get n and do GOTO n
...
|;    BSR          LAB_1212                 |; check room on stack for A bytes
...
LAB_GOTO:
    ...
    BLS     LAB_16D0                        |; branch if current |; => wanted #

    MOVEA.l %a5,%a0                         |; copy BASIC execute pointer
LAB_GOTs:
    TST.b   (%a0)+                          |; test next byte & increment pointer
    BNE     LAB_GOTs                        |; loop if not EOL
...
LAB_LOOP:
    ...
    MOVE.L  #-1,%d7                         |; set invert result longword
DoRest:
    BSR     LAB_IGBY                        |; increment & scan memory
    ...
    MOVE.b  #0xFF,FAC1_e(%a3)               |; else set all bits
DoCmp:
...
LAB_RETURN:
    BNE     RTS_007                         |; exit if following token to allow syntax error
...
LAB_SNBS:
    MOVEA.l %a5,%a0                         |; copy BASIC execute pointer
    MOVE.L  #0x22,%d1                       |; set string quote character
    MOVE.L  #0x3A,%d2                       |; set look for character = ":"
    BRA     LAB_172D                        |; go do search
...
LAB_172C:
    ...
    BEQ     LAB_1725                        |; if found go search for [EOL]

LAB_172D:
    MOVE.b  (%a0)+,%d0                      |; get next byte
    BNE     LAB_172C                        |; loop if not null [EOL]
...
LAB_1723:
    CMP.b   %d0,%d1                         |; compare with '"'
    BEQ     LAB_172D                        |; if found go search for ":" or [EOL]

LAB_1725:
    MOVE.b  (%a0)+,%d0                      |; get next byte
    BNE     LAB_1723                        |; loop if not null [EOL]
...
LAB_IF:
    ...
    BEQ     LAB_174B                        |; if it was THEN then continue
...
LAB_174B:
    ...
    BEQ     LAB_174E                        |; if result was zero go look for an ELSE
...
LAB_174E:
    ...
    MOVE.L  #TK_ELSE,%d3                    |; set search for ELSE token
    MOVE.L  #TK_IF,%d4                      |; set search for IF token
    MOVE.L  #0,%d5                          |; clear the nesting depth
LAB_1750:
    MOVE.b  (%a5)+,%d0                      |; get next BASIC byte & increment ptr
    BEQ     LAB_1754                        |; if EOL correct the pointer and return

    CMP.b   %d4,%d0                         |; compare with "IF" token
    BNE     LAB_1752                        |; skip if not nested IF

    ADDQ.w  #1,%d5                          |; else increment the nesting depth ..
    BRA     LAB_1750                        |; .. and continue looking

LAB_1752:
    CMP.b   %d3,%d0                         |; compare with ELSE token
    BNE     LAB_1750                        |; if not ELSE continue looking
...
LAB_GFPN:
    MOVE.L  #0x00,%d1                       |; clear integer register
    MOVE.l  %d1,%d0                         |; clear d0
    BSR     LAB_GBYT                        |; scan memory, Cb=1 if "0"-"9", & get byte
    BCC     LAB_1786                        |; return if carry clear, chr was not "0"-"9"
...
LAB_1785:
    ...
    BSR     LAB_IGBY                        |; increment & scan memory
    BCS     LAB_1785                        |; loop for next character if "0"-"9"
...
|; value at the top of this file to some non zero value
 ...
 .ifndef    novar
    ...
    BEQ     LAB_INCT                        |; if variable not found skip the inc/dec

 .endif

    TST.b   Dtypef(%a3)                     |; test data type, 0x80=string, 0x40=integer,
                                            |; 0x00=float
    BMI     LAB_TMER                        |; if string do "Type mismatch" error/warm start

    BNE     LAB_INCI                        |; go do integer INC/DEC

    MOVE.l  %a0,Lvarpl(%a3)                 |; save var address
    BSR     LAB_UFAC                        |; unpack memory (%a0) into FAC1
    MOVE.l  #0x80000000,FAC2_m(%a3)         |; set FAC2 mantissa for 1
...
LAB_INCT:
    ...
    BEQ     LAB_17B8                        |; continue if "," (another variable to do)

    ADDQ.w  #2,%sp                          |; else dump sign & exponent
    RTS
LAB_INCI:
    TST.b   1(%sp)                          |; test sign
    BNE     LAB_DECI                        |; branch if DEC

    ADDQ.l  #1,(%a0)                        |; increment variable
    BRA     LAB_INCT                        |; go scan for more
...
LAB_LET:
    ...
    MOVE.b  Dtypef(%a3),-(%sp)              |; push var data type, 0x80=string, 0x40=integer,
                                            |; 0x00=float
...
LAB_17D6:
    ...
    CMP.l   Sstorl(%a3),%a1                 |; compare string memory start with string
                                            |; pointer
    BCS     LAB_1811                        |; if it was in program memory assign the value
                                            |; and exit

    CMPA.l  Sfncl(%a3),%a0                  |; compare functions start with descriptor
                                            |; pointer
    BCS     LAB_1811                        |; branch if >= (string is on stack)

                                            |; string is variable$ make space and copy string
LAB_1810:
    MOVE.L  #0,%d1                          |; clear length
...
LAB_GETS:
    MOVE.L  #0x00,%d1                       |; assume no byte
    MOVE.l  %d1,%a0                         |; assume null string
    BSR     INGET                           |; get input byte
    BCC     LAB_NoSt                        |; branch if no byte received

    MOVE.L  #0x01,%d1                       |; string is single byte
...
LAB_NoSt:
    ...
    BRA     LAB_17D5                        |; do string LET & return
...
|;*    MOVE.b    Dtypef(%a3),%d0            |; get data type flag, 0x80=string, 0x00=numeric
|;*    BMI          LAB_1829                |; branch if string
...
LAB_CRLF:
    MOVE.L  #0x0D,%d0                       |; load [CR]
    BSR     LAB_PRNA                        |; go print the character
    MOVE.L  #0x0A,%d0                       |; load [LF]
    BRA     LAB_PRNA                        |; go print the character & return

LAB_188B:
    MOVE.b  TPos(%a3),%d2                   |; get terminal position
    CMP.b   Iclim(%a3),%d2                  |; compare with input column limit
    BCS     LAB_1898                        |; branch if less than Iclim

    BSR     LAB_CRLF                        |; else print CR/LF (next line)
    BRA     LAB_18BD                        |; continue with PRINT processing

LAB_1898:
    SUB.b   TabSiz(%a3),%d2                 |; subtract TAB size
    BCC     LAB_1898                        |; loop if result was >= 0

    NEG.b   %d2                             |; twos complement it
    BRA     LAB_18B7                        |; print %d2 spaces
...
|; calculate TAB offset
    ...
    BLS     LAB_18BD                        |; branch if result was <= 0
...
LAB_PRNA:
    ...
    BCS     LAB_18F9                        |; branch if less, non printing character
    ...
    BRA     LAB_18F7                        |; go print character
...
LAB_18F7:
    ADDQ.b  #0x01,TPos(%a3)                 |; increment terminal position
LAB_18F9:
...
LAB_1904:
    ...
    TST.b   Imode(%a3)                      |; test input mode flag, 0x00=INPUT, 0x98=READ
...
LAB_INPUT:
    ...
    BNE     LAB_1934                        |; if no prompt string just go get the input

    BSR     LAB_1BC1                        |; print "..." string
    MOVE.L  #';',%d0                        |; set the search character to ";"
    BSR     LAB_SCCA                        |; scan for CHR$(%d0), else do syntax error/warm
                                            |; start
...
|; value at the top of this file to some non zero value
 ...
 .ifndef    nobrk
    ...
    BRA     LAB_1953                        |; go handle the input
...
LAB_READ:
    ...
    MOVE.L  #0x98-0x100,%d0                 |; flag READ
LAB_1953:
    MOVE.b  %d0,Imode(%a3)                  |; set input mode flag, 0x00=INPUT, 0x98=READ
...
LAB_1961:
    ...
    BSR     LAB_GBYT                        |; scan memory
    BNE     LAB_1986                        |; if not null go get the value

|; the pointer was to a null entry
    TST.b   Imode(%a3)                      |; test input mode flag, 0x00=INPUT, 0x98=READ
    BMI     LAB_19DD                        |; branch if READ (go find the next statement)
...
|; value at the top of this file to some non zero value
 ...
 .ifndef    nobrk
    ...
    BRA     LAB_1961                        |; go handle the input
...
|; value at the top of this file set to zero
 ...
 .ifdef    nobrk
    ...
    BNE     LAB_1984                        |; if not null input go handle it
...
LAB_1986:
    ...
    MOVE.L  #':',%d2                        |; set new search character
    MOVE.L  #',',%d0                        |; other search character is ","
...
LAB_NEXT:
    ...
    BRA     LAB_11BD                        |; branch always (no variable to search for)
...
LAB_1A46:
    ...
    MOVE.L  #0x1C,%d1                        |; set for FOR use size
    BRA     LAB_11A6                        |; enter loop for next variable search
...
LAB_11A6:
    ...
    BNE     LAB_11A5                        |; loop if no match found
...
LAB_11BD:
    ...
    TST.b   %d0                |; test for =
    BEQ     LAB_1A90                        |; branch if = (loop INcomplete)

    TST.b   %d1                |; test result
    BPL     LAB_1A9B                        |; branch if > (loop complete)
...
LAB_CKTM:
    ...
    BNE     LAB_1ABA                        |; branch if data type is string

                            |; else data type was numeric
    BCS     LAB_TMER                        |; if required type is string do type mismatch
                            |; error

    RTS     |; data type was string, now check required type
LAB_1ABA:
...
LAB_EVEX:
    SUBQ.w  #1,%a5                          |; decrement BASIC execute pointer
LAB_EVEZ:
    MOVE.L  #0,%d1                          |; clear precedence word
    MOVE.b  %d1,Dtypef(%a3)                 |; clear the data type, 0x80=string, 0x40=integer,
                                            |; 0x00=float
    BRA     LAB_1ACD                        |; enter loop
...
LAB_1ACD:
    ...
    BSR     LAB_GVAL                        |; get value from line
    MOVE.b  #0x00,comp_f(%a3)               |; clear compare function flag
LAB_1ADB:
    BSR     LAB_GBYT                        |; scan memory
LAB_1ADE:
    SUB.b   #TK_GT,%d0                      |; subtract token for > (lowest compare function)
    BCS     LAB_1AFA                        |; branch if < TK_GT

    CMP.b   #0x03,%d0                       |; compare with ">" to "<" tokens
    BCS     LAB_1AE0                        |; branch if <= TK_SGN (is compare function)
...
LAB_1B2A:
    ...
    MOVE.b  #0,Dtypef(%a3)                  |; clear data type flag, 0x00=float
    MOVE.b  %d1,comp_f(%a3)                 |; save new compare function flag
    SUBQ.w  #1,%a5                          |; decrement BASIC execute pointer
    MOVE.L  #(TK_LT-TK_PLUS)*4,%d1          |; set offset to last operator entry
|    move.l  #TK_LT,%d1
|    subi.l  #TK_PLUS,%d1
|    lsr.l   #4,%d1
...
LAB_1B3C:
    ...
    CMP.w   (%a0,%d1.w),%d0                 |; compare with this opperator precedence
    BCC     LAB_1B86                        |; branch if %d0 >=, pop FAC2 & return
...
LAB_1B7D:
    ...
    BEQ     LAB_1B84                        |; branch if was 0x64 (< function can be string)
...
LAB_GVAL:
    ...
    BNE     LAB_1BF3                        |; if not open quote it must be a variable or
                                            |; open bracket
...
LAB_1BC1:
    ...
    RTS                                     |; get value from line .. continued
...
LAB_SCCA:
    ...
    BEQ     LAB_GBYT                        |; if so go get next
...
LAB_1C11:
    BSR     LAB_CTNM                        |; check if source is numeric, else type mismatch
    MOVE.L  #(TK_GT-TK_PLUS)*4,%d1          |; set offset from base to - operator
LAB_1C13:
...
|; value at the top of this file to some non zero value
 ...
 .ifndef    novar
    ...
    BNE     LAB_1C19                        |; if it exists return it
...
LAB_1C19:
    ...
    BPL     LAB_1C1A                        |; if integer unpack memory (%a0) into FAC1
                                            |; and return
...
LAB_1BD0:
    CMP.b   #TK_MINUS,%d0                   |; compare with token for -
    BEQ     LAB_1C11                        |; branch if - token (do set-up for - operator)
    ...
    BRA     LAB_1C13                        |; do set-up for function then execute
...
|; for functions that returned strings
    ...
    BEQ     LAB_1C2A                        |; no preprocess vector so go do function
...
LAB_PPBI:
    MOVE.b  #0x00,Dtypef(%a3)               |; clear data type flag, 0x00=float
...
LAB_BHSS:
    ...
    MOVE.L  #0,%d1                          |; set default to no leading "0"s
    MOVE.b  (%a5)+,%d2                      |; get BASIC byte
    CMP.b   #',',%d2                        |; compare with comma
...
LAB_LTHAN:
    ...
    BSR     LAB_27FA                        |; compare FAC1 with FAC2
                                            ...
                                            |; returns d0= 0 if FAC1 = FAC2
                                            |; returns d0=-1 if FAC1 < FAC2
    BRA     LAB_1CF2                        |; process result

                                            |; do string < compare
LAB_1CAE:
    MOVE.b  #0x00,Dtypef(%a3)               |; clear data type, 0x80=string, 0x40=integer,
                                            |; 0x00=float
    ...
    BNE     LAB_1CB5                        |; branch if not null string

    TST.l   %d1                             |; test if string 2 is null also
    BEQ     LAB_1CF2                        |; if so do string 1 = string 2

LAB_1CB5:
    SUB.l   %d1,%d2                         |; subtract string 2 length
    BEQ     LAB_1CD5                        |; branch if strings = length

    BCS     LAB_1CD4                        |; branch if string 1 < string 2

    MOVE.L  #-1,%d0                         |; set for string 1 > string 2
    BRA     LAB_1CD6                        |; go do character comapare

LAB_1CD4:
    MOVE.l  %d0,%d1                         |; string 1 length is compare length
    MOVE.L  #1,%d0                          |; and set for string 1 < string 2
    BRA     LAB_1CD6                        |; go do character comapare
...
LAB_1CE6:
    ...
    BEQ     LAB_1CF2                        |; if = to here, then go use length compare

    BCC     LAB_1CDB                        |; else branch if string 1 > string 2

    MOVE.L  #-1,%d0                         |; else set for string 1 < string 2
    BRA     LAB_1CF2                        |; go set result
...
LAB_DIM:
    MOVE.L  #-1,%d1                         |; set "DIM" flag
    BSR     LAB_1D10                        |; search for or dimension a variable
    BSR     LAB_GBYT                        |; scan memory
    BNE     LAB_1CFE                        |; loop and scan for "," if not null
...
LAB_RSHIFT:
    ...
    CMP.b   #0x20,%d2                       |; compare bit count with 32d
    BCS     Not2Big                         |; branch if >= (return shift)

    TST.l   %d0                             |; test sign bit
    BPL     TooBig                          |; branch if +ve

    MOVE.L  #-1,%d0                         |; set longword
...
LAB_SVAR:
    BSR     LAB_GVAR                        |; search for variable
LAB_FVAR:
    RTS     
/***********************************************************************************/
#
...
LAB_GVAR:
    MOVE.L  #0x00,%d1                       |; set DIM flag = 0x00
...
LAB_1D12:
    BSR     LAB_CASC                        |; check byte, return C=0 if<"A" or >"Z"
    BCC     LAB_SNER                        |; if not, syntax error then warm start

                                            |; it is a variable name so ...
    MOVE.L  #0x0,%d1                        |; set index for name byte
...
LAB_1D2D:
    ...
    BCC     LAB_1D2E                        |; if so go ignore any more

    MOVE.b  %d0,(%a0,%d1.w)                 |; save the character
    ADDQ.w  #1,%d1                          |; increment index
LAB_1D2E:
    BSR     LAB_IGBY                        |; increment & scan memory (next character)
    BCS     LAB_1D2D                        |; branch if character = "0"-"9" (ok)

                                            |; character wasn't "0" to "9" so ...
    BSR     LAB_CASC                        |; check byte, return C=0 if<"A" or >"Z"
    BCS     LAB_1D2D                        |; branch if = "A"-"Z" (ok)

                                            |; check if string variable
    CMP.b   #'$',%d0                        |; compare with "$"
    BNE     LAB_1D44                        |; branch if not string

                                            |; type is string
    OR.b        #0x80,Varname+1(%a3)        |; set top bit of 2nd character, indicate string
    BSR     LAB_IGBY                        |; increment & scan memory
    BRA     LAB_1D45                        |; skip integer check

                                            |; check if integer variable
LAB_1D44:
    CMP.b   #'&',%d0                        |; compare with "&"
    BNE     LAB_1D45                        |; branch if not integer

                                            |; type is integer
    OR.b        #0x80,Varname+2(%a3)        |; set top bit of 3rd character, indicate integer
    BSR     LAB_IGBY                        |; increment & scan memory
...
LAB_1D45:
    ...
    BEQ     LAB_1D48                        |; if not FN or FN variable continue

    BPL     LAB_1D49                        |; if FN variable go find or create it

                                            |; else was FN name
    MOVE.l  Varname(%a3),%d0                |; get whole function name
    MOVE.L  #8,%d1                          |; set step to next function size -4
    LEA     Sfncl(%a3),%a0                  |; get pointer to start of functions
    BRA     LAB_1D4B                        |; go find function
...
LAB_1D4B:
    ...
    BRA     LAB_1D5E                        |; enter loop at exit check
...
LAB_1D5E:
    ...
    BNE     LAB_1D5D                        |; if not end go check next

    TST.b   Sufnxf(%a3)                     |; is it a function or function variable
    BNE     LAB_1D94                        |; if was go do DEF or function variable
...
LAB_1D98:
    ...
    BEQ     LAB_1DAF                        |; skip move if zero length block
...
LAB_1DAF:
    ...
    BMI     LAB_1DB0                        |; branch if was FN

    BTST.l  #23,%d0                        |; was it string
    BNE     LAB_1DB2                        |; branch if string

    BRA     LAB_1DB1                        |; branch if was plain variable
...
LAB_1DB2:
    ...
    BTST.l  #23,%d0                        |; was it string
    BEQ     LAB_1DD7                        |; branch if not string
...
LAB_1DD7:
    ...
    MOVE.b  #0x00,Sufnxf(%a3)        |; clear FN flag byte
...
|; value at the top of this file to some non zero value
 ...
 .ifndef    novar
    ...
    MOVE.L  #-1,%d0                        |; return variable found
...
LAB_1DE6:
    MOVE.L  #5,%d0                |; set %d0 to 5 (*2 = 10, later)
...
LAB_EVIR:
    CMPI.b  #0xA0,FAC1_e(%a3)        |; compare exponent with exponent = 2^32 (n>2^31)
    ...
    NEG.l   %d0                |; do -d0
    BVC     LAB_FCER                        |; if not 0x80000000 do FC error, then warm start
...
LAB_1E17:
    ...
    MOVE.L  #0,%d1                |; clear dimensions count
...
LAB_1E1F:
    ...
    BSR     LAB_EVIN                        |; evaluate integer expression
...
LAB_1E5C:
    ...
    BRA     LAB_1E5C                        |; go check next array
...
LAB_1EA1:
    ...
    MOVE.L  #4,%d1                |; set 4 bytes per element
    BTST.l  #23,%d0                        |; test if string array
    BEQ     LAB_1EDF                        |; branch if not string

    MOVE.L  #6,%d1                |; else 6 bytes per element
LAB_1EDF:
...
|;    BNE          LAB_1ED0                        |; branch if b6 of Defdim is clear
...
#LAB_1ED0
    ...
    BCS     LAB_1ED6                        |; branch if less (is ok)
...
LAB_1ED8:
    ...
    BEQ     LAB_1F07                        |; exit if done

    SUBQ.w  #1,%d1                |; decrement low (high) word
    SWAP    %d1                |; swap back
    BRA     LAB_1ED8                        |; go do a whole block
...
LAB_1F07:
    ...
    TST.b   Defdim(%a3)                        |; test default DIM flag
    BNE     RTS_011                        |; exit (RET) if this was a DIM command
...
LAB_1F28:
    MOVE.L  #0,%d0                |; clear first result
...
|; now do pointer = pointer |; nth dimension + nth index
    ...
    BEQ     LAB_1F5A                        |; skip multiply if last result = null
...
LAB_1F5A:
    ...
    MOVE.L  #3,%d1                |; set for numeric array
    TST.b   Varname+1(%a3)        |; test if string array
    BPL     LAB_1F6A                        |; branch if not string

    MOVE.L  #5,%d1                |; else set for string array
    MOVE.b  #0x80,Dtypef(%a3)        |; and set data type to string
    BRA     LAB_1F6B                        |; skip integer test

LAB_1F6A:
    TST.b   Varname+2(%a3)        |; test if integer array
    BPL     LAB_1F6B                        |; branch if not integer

    MOVE.b  #0x40,Dtypef(%a3)        |; else set data type to integer
LAB_1F6B:
    BSR     LAB_1F7C                        |; do element size (%d1) |; array size (Asptl)
...
LAB_AYFC:
    MOVE.b  #0x00,Dtypef(%a3)               |; clear data type, 0x80=string, 
                                            |; 0x40=integer, 0x00=float
    MOVE.w  #0xA000,FAC1_e(%a3)             |; set FAC1 exponent and clear sign (b7)
...
LAB_DEF:
    ...
    MOVE.b  #0x80,Sufnxf(%a3)        |; set FN flag bit
    ...
    MOVE.b  #0x7E,Sufnxf(%a3)        |; set FN variable flag bits
    BSR     LAB_SVAR                        |; search for or create a variable
                            |; return the variable address in a0
    BSR     LAB_1BFB                        |; scan for ")", else do syntax error/warm start
...
LAB_201E:
    MOVE.b  #0x81,Sufnxf(%a3)        |; set FN flag (find not create)
    BSR     LAB_IGBY                        |; increment & scan memory
    BSR     LAB_1D12                        |; get FN name
    ...
    BSR     LAB_CKTM                        |; type match check, set C for string
                            ...
                            |; use
    BEQ     LAB_2043                        |; branch if not string
...
LAB_204S:
    ...
    BSR     LAB_2045                        |; pack function expression value into (%a0)
                            |; (function variable)
    MOVE.l  %a5,-(%sp)                        |; push BASIC execute pointer
...
LAB_20AE:
    MOVE.L  #0x22,%d2                        |; set Srchc character (terminator 1)
...
LAB_20BE:
    ...
    BEQ     LAB_20D0                        |; exit loop if null byte [EOS]
    ...
    BNE     LAB_20BE                        |; loop if not terminator 2 (or null string)

LAB_20CB:
    CMP.b   #0x22,%d0                        |; compare with "
    BNE     LAB_20D0                        |; branch if not "
...
LAB_20D0:
    ...
    BCS     LAB_RTST                        |; if not go push descriptor on stack & exit
                            |; (could be message string from ROM)

    CMPA.l  Smeml(%a3),%a0        |; is string in utility ram
    BCC     LAB_RTST                        |; if not go push descriptor on stack & exit
...
LAB_20C9:
    ...
    BNE     LAB_20D8                        |; branch if not null string

    MOVEA.l %d1,%a0                |; make null pointer
    BRA     LAB_RTST                        |; go push descriptor on stack & exit
...
LAB_RTST:
    ...
    MOVE.b  #0x80,Dtypef(%a3)        |; save data type flag, 0x80=string
    RTS     
/***********************************************************************************/
#
...
LAB_2115:
    ...
    BEQ     LAB_2128                        |; branch if user wants null string

                            |; make space for string %d1 long
    MOVE.l  %d0,-(%sp)                        |; save d0
    MOVE.L  #0,%d0                |; clear longword
    MOVE.b  %d0,Gclctd(%a3)        |; clear garbage collected flag (b7)
    MOVE.L  #1,%d0                |; +1 to possibly round up
    AND.w   %d1,%d0                |; mask odd bit
    ADD.w   %d1,%d0                |; ensure %d0 is even length
    BCC     LAB_2117                        |; branch if no overflow

    MOVE.L  #1,%d0                |; set to allocate 65536 bytes
...
LAB_2117:
    ...
    BCS     LAB_2137                        |; if less do out of memory error
...
LAB_2137:
    ...
    BSR     LAB_GARB                        |; else go do garbage collection
    MOVEA.l (%sp)+,%a1                        |; restore a1
    MOVE.b  #0x80,Gclctd(%a3)        |; set garbage collected flag
    BRA     LAB_2117                        |; go try again
...
LAB_214B:
    ...
    MOVEA.l Sarryl(%a3),%a2        |; set end pointer to start of arrays (end of
                            |; strings)
    BRA     LAB_2176                        |; branch into loop at end loop test
...
LAB_2176:
    ...
    BCS     LAB_2161                        |; go do next if not at end
...
|; done strings, now do arrays.
    ...
    BRA     LAB_218F                        |; branch into loop at end loop test
...
LAB_217E:
    ...
    BEQ     LAB_218B                        |; branch if not string
...
LAB_2183:
    ...
    BNE     LAB_2183                        |; go do next if not at end of array

LAB_218B:
    MOVEA.l %d2,%a0                |; pointer to next array
LAB_218F:
    CMPA.l  %a0,%a2                |; compare pointer with array end
    BNE     LAB_217E                        |; go do next if not at end
...
|; done arrays and variables, now just the descriptor stack to do
    ...
    BRA     LAB_21C4                        |; branch into loop at end loop test
...
LAB_21D1:
    ...
    RTS     |; test and set if this is the highest string
...
LAB_2206:
    ...
    BEQ     LAB_2212                        |; if = go move string store pointer down
...
LAB_224E:
    ...
    MOVEA.l (%a0),%a0                        |; get string pointer
    BSR     LAB_229E                        |; copy string %d0 bytes long from %a0 to Sutill
...
LAB_CHRS:
    BSR     LAB_EVBY                        |; evaluate byte expression, result in %d0 and
                            |; Itemp
LAB_MKCHR:
    MOVE.L  #1,%d1                |; string is single byte
    BSR     LAB_2115                        |; make string space %d1 bytes long
...
LAB_LEFT:
    ...
    BEQ     LAB_231C                        |; branch if null return

    MOVE.L  #0,%d0                |; clear start offset
    CMP.w   4(%a0),%d1                        |; compare word parameter with string length
    BCS     LAB_231C                        |; branch if string length > word parameter

    BRA     LAB_2317                        |; go copy whole string
...
LAB_RIGHT:
    ...
    BCC     LAB_231C                        |; branch if string length > word parameter
...
LAB_MIDS:
    MOVE.L  #0,%d7                |; clear longword
    SUBQ.w  #1,%d7                |; set default length = 65535
    MOVE.l  %d0,-(%sp)                        |; save word 1
    BSR     LAB_GBYT                        |; scan memory
    CMP.b   #',',%d0                        |; was it ","
    BNE     LAB_2358                        |; branch if not "," (skip second byte get)
...
LAB_2358:
    ...
    BCC     LAB_231C                        |; if start not in string do null string (d1=0)

    MOVE.l  %d7,%d1                |; get length back
    ADD.w   %d0,%d7                |; %d7 now = MID$() end
    BCS     LAB_2368                        |; already too long so do RIGHT$ equivalent

    CMP.w   4(%a0),%d7                        |; compare string length with start index+length
    BCS     LAB_231C                        |; if end in string go do string

LAB_2368:
    MOVE.w  4(%a0),%d1                        |; get string length
    SUB.w   %d0,%d1                |; subtract start offset
    BRA     LAB_231C                        |; go do string (effectively RIGHT$)
...
LAB_VAL:
    BSR     LAB_22B6                        |; pop string off descriptor stack or from memory
                            |; returns with %d0 = length, %a0 = pointer
    BEQ     LAB_VALZ                        |; string was null so set result = 0x00
...
LAB_SWAP:
    ...
    MOVE.b  Dtypef(%a3),%d4        |; copy variable 1 data type, 0x80=string,
...
|; value at the top of this file set to zero
 ...
 .ifdef    novar
    ...
    BPL     RTS_015                        |; exit if not string
...
|; value at the top of this file to some non zero value
 ...
 .ifndef    novar
    ...
    BEQ     no_variable1        |; if variable 1 doesn't exist skip the
                            |; value get

    MOVE.l  (%a2),%d2                        |; get variable 1 value
    TST.b   %d4                |; check the data type
    BPL     no_variable1        |; if not string skip the length get
...
no_variable1:
    ...
    BEQ     no_variable2        |; if variable 2 doesn't exist skip the
                            |; value get and the new value save

    MOVE.l  (%a0),%d0                        |; get variable 2 value
    MOVE.l  %d2,(%a0)+                        |; save variable 2 new value
    TST.b   %d4                |; check the data type
    BPL     no_variable2        |; if not string skip the length get and
                            |; new length save
...
no_variable2:
    ...
    BEQ     EXIT_SWAP                        |; if variable 1 doesn't exist skip the
                            |; new value save

    MOVE.l  %d0,(%a2)+                        |; save variable 1 new value
    TST.b   %d4                |; check the data type
    BPL     EXIT_SWAP                        |; if not string skip the new length save
...
LAB_SUBTRACT:
    EORI.b  #0x80,FAC1_s(%a3)               |; complement FAC1 sign
...
LAB_ADD:
    ...
    BCS     LAB_249C                        |; branch if FAC2 < FAC1
...
LAB_249C:
    ...
    BLT     LAB_2467                        |; branch if range >= 32

    MOVE.L  #0,%d1                          |; clear d1
    BRA     LAB_2468                        |; go clear smaller mantissa
...
LAB_24A8:
    ...
    BMI     LAB_24F8                        |; if <> go do subtract

    MOVE.l  FAC2_m(%a3),%d0                 |; get FAC2 mantissa
    ADD.l   FAC1_m(%a3),%d0                 |; add FAC1 mantissa
    BCC     LAB_24F7                        |; save and exit if no carry (FAC1 is normal)
...
LAB_24D0:
    BCC     LAB_24D5                        |; branch if result is +ve

                                            |; erk! subtract is the wrong way round so
                                            |; negate everything
    EORI.b  #0xFF,FAC1_s(%a3)               |; complement FAC1 sign
...
LAB_24D5:
    MOVE.l  FAC1_m(%a3),%d0                 |; get mantissa
    BMI     LAB_24DA                        |; mantissa is normal so just exit

    BNE     LAB_24D9                        |; mantissa is not zero so go normalise FAC1
...
LAB_24D9:
    ...
    BEQ     LAB_24D8                        |; if exponent is zero then clean up and exit
LAB_24D6:
    ...
    BEQ     LAB_24D8                        |; if exponent is zero make FAC1 zero

    BPL     LAB_24D7                        |; if exponent is >zero go save FAC1

    MOVE.L  #1,%d0                          |; else set for zero after correction
LAB_24D7:
...
LAB_LOG:
    ...
    MOVE.b  #0x81,FAC1_e(%a3)        |; force a value between 1 and 2
    MOVE.l  FAC1_m(%a3),%d6        |; copy mantissa

    MOVE.l  #0x80000000,FAC2_m(%a3)    |; set mantissa for 1
    MOVE.w  #0x8100,FAC2_e(%a3)        |; set exponent for 1
    BSR     LAB_ADD                        |; find arg+1
    MOVE.L  #0,%d0                |; setup for calc skip
    MOVE.w  %d0,FAC2_e(%a3)        |; set FAC1 for zero result
    ADD.l   %d6,%d6                |; shift 1 bit out
    MOVE.l  %d6,FAC2_m(%a3)        |; put back FAC2
    BEQ     LAB_LONN                        |; if 0 skip calculation

    MOVE.w  #0x8000,FAC2_e(%a3)        |; set exponent for .5
...
|; %a0 = table pointer
    ...
    MOVE.L  #30,%d5                        |; loop 31 times
    MOVE.L  #1,%d4                |; set shift count
    BRA     LAB_LOCC                        |; entry point for loop
...
LAB_LOLP:
    SUB.l   %d3,%d0                |; x = x - y1
    BPL     LAB_LAAD                        |; branch if > 0

    MOVE.l  %d2,%d0                |; get x back
    ADDQ.w  #4,%a0                |; next entry
    ADDQ.l  #1,%d4                |; next i
    LSR.l   #1,%d3                |; /2
    BEQ     LAB_LOCX                        |; branch y1 = 0
...
LAB_LONN:
    ...
    MOVE.L  #0,%d5                |; set default exponent sign
    TST.l   %d7                |; check original exponent sign
    BEQ     LAB_LOXO                        |; branch if original was 0

    BPL     LAB_LOXP                        |; branch if was +ve

    NEG.l   %d7                |; make original exponent +ve
    move.l  #0x80-0x100,%d5        |; make sign -ve
LAB_LOXP:
    MOVE.b  %d5,FAC1_s(%a3)        |; save original exponent sign
    SWAP    %d7                |; 16 bit shift
    LSL.l   #8,%d7                |; easy first part
    move.l  #0x88-0x100,%d5        |; start with byte
LAB_LONE:
    SUBQ.l  #1,%d5                |; decrement exponent
    ADD.l   %d7,%d7                |; shift mantissa
    BPL     LAB_LONE                        |; loop if not normal

LAB_LOXO:
    MOVE.l  %d7,FAC1_m(%a3)        |; save original exponent as mantissa
    MOVE.b  %d5,FAC1_e(%a3)        |; save exponent for this
    MOVE.l  #0xB17217F8,FAC2_m(%a3)    |; LOG(2) mantissa
    MOVE.w  #0x8000,FAC2_e(%a3)        |; LOG(2) exponent & sign
    MOVE.b  FAC1_s(%a3),FAC_sc(%a3)    |; make sign compare = FAC1 sign
    BSR     LAB_MULTIPLY        |; do multiply
    MOVE.l  %d4,FAC2_m(%a3)        |; save cordic result
    BEQ     LAB_LOWZ                        |; branch if zero

    MOVE.w  #0x8200,FAC2_e(%a3)        |; set exponent & sign
...
LAB_MULTIPLY:
    ...
    BCC     LAB_MNOC                        |; branch if no carry
...
LAB_MUF1:
    ...
    BCC     LAB_MUF2                        |; branch if no carry
...
LAB_DIVIDE:
    ...
    MOVE.L  #0,%d0                |; clear FAC2 exponent
    ...
    MOVE.b  FAC2_e(%a3),%d0        |; get FAC2 exponent
    BEQ     LAB_DIV0                        |; if zero return zero
...
LAB_XDIV:
    ...
    RTS     |; FAC1 mantissa = FAC2 mantissa so set result mantissa

LAB_MAN1:
    MOVE.L  #1,%d1                |; set bit
    LSR.l   %d1,%d1                |; bit into eXtend
    BRA     LAB_SET1                        |; set mantissa, adjust exponent and exit

|; result is zero

LAB_DIV0:
    MOVE.L  #0,%d0                |; zero exponent & sign
...
|; %d4            AAAA    BBBB                        |; numerator
    ...
    BVC     LAB_LT_1                        |; if no overflow DIV was ok

    MOVE.L  #-1,%d4                        |; else set default value
...
|; %d4            QQQQ    RRRR                        |; 48 bit result partial high longword
    ...
    MOVE.L  #0,%d6                |; clear to extend numerator to 48 bits
...
|; now if we got the divide correct then the remainder high longword will be +ve
    ...
    BPL     L_DDIV                        |; branch if result is ok (<needed)
...
|; %d2            GGGG    HHHH                        |; remainder high longword
    ...
    BMI     LAB_REMM                        |; loop if result still too big
...
LAB_UFAC:
    ...
    BEQ     LAB_NB1T                        |; branch if exponent (and the rest) zero
...
LAB_PFAC:
    ...
    BEQ     LAB_277C                        |; branch if floating
...
LAB_277C:
    ...
    ANDI.b  #0x80,FAC1_s(%a3)               |; clear non sign bits in sign
...
LAB_27BA:
    MOVE.b  FAC1_e(%a3),%d0                 |; get FAC1 exponent
    BEQ     LAB_27C4                        |; branch if zero

    MOVE.l  FAC1_m(%a3),%d0                 |; get FAC1
    ADD.l   #0x80,%d0                       |; round to 24 bit
    BCC     LAB_27C3                        |; branch if no overflow
...
LAB_27D0:
    ...
    BCS     RTS_017                         |; exit if carry set

    MOVE.L  #1,%d0                          |; set result for +ve sign
    RTS     
/***********************************************************************************/
#
|; perform SGN()
...
LAB_27DB:
    MOVE.l  %d0,FAC1_m(%a3)                 |; save FAC1 mantissa
    MOVE.w  #0xA000,FAC1_e(%a3)             |; set FAC1 exponent & sign
...
LAB_27FA:
    ...
    BEQ     LAB_27CA                        |; branch if FAC2 exponent=0 & get FAC1 sign
                            |; d0=-1,C=1/-ve d0=+1,C=0/+ve

    MOVE.b  FAC_sc(%a3),%d0        |; get FAC sign compare
    BMI     LAB_27CE                        |; if signs <> do return d0=-1,C=1/-ve
                            |; d0=+1,C=0/+ve & return
    ...
    BEQ     LAB_282F                        |; exit if mantissas equal

|; gets here if number <> FAC1

LAB_2828:
    BCS     LAB_27D0                        |; if FAC1 > FAC2 return d0=-1,C=1/-ve d0=+1,
                            |; C=0/+ve

    EORI.b  #0x80,%d0                        |; else toggle FAC1 sign
LAB_282E:
    BRA     LAB_27D0                        |; return d0=-1,C=1/-ve d0=+1,C=0/+ve
...
LAB_2831:
    ...
    BEQ     LAB_284J                        |; branch if mantissa = 0
    ...
    BNE     LAB_284G                        |; branch if exponent was not 0xA0

    TST.b   FAC1_s(%a3)                        |; test FAC1 sign
    BPL     LAB_284H                        |; branch if FAC1 +ve

    NEG.l   %d0
    BVS     LAB_284H                        |; branch if was 0x80000000
...
LAB_INT:
    ...
    SUB.b   FAC1_e(%a3),%d0        |; subtract FAC1 exponent
    BLS     LAB_IRTS                        |; exit if exponent >= 0xA0
...
LAB_2970:
    ...
    MOVE.L  #' ',%d2                        |; character = " ", assume +ve
    BCLR.b  #7,FAC1_s(%a3)        |; test and clear FAC1 sign (b7)
    BEQ     LAB_2978                        |; branch if +ve

    MOVE.L  #'-',%d2                        |; else character = "-"
LAB_2978:
    ...
    MOVE.L  #'0',%d0                        |; set character = "0"
    MOVE.L  #1,%d1                |; set output string index
...
LAB_2989:
    ...
    BCC     LAB_299C                        |; branch if FAC1=>1

                            |; else FAC1 < 1
    MOVE.l  #0x98968000,FAC2_m(%a3)    |; 10000000 mantissa
    MOVE.w  #0x9800,FAC2_e(%a3)        |; 10000000 exponent & sign
    MOVE.b  FAC1_s(%a3),FAC_sc(%a3)    |; make FAC1 sign sign compare
    BSR     LAB_MULTIPLY        |; do FAC2*FAC1

    MOVE.b  #0xF9,numexp(%a3)        |; set number exponent count (-7)
    BRA     LAB_299C                        |; go test for fit
...
LAB_29B9:
    ...
    MOVE.l  #0xCCCCCCCD,FAC1_m(%a3)    |; 1/10 mantissa
    MOVE.w  #0x7D00,FAC1_e(%a3)        |; 1/10 exponent & sign
    BSR     LAB_MULTIPLY        |; do FAC2*FAC1, effectively divide by 10 but
                            |; faster

    ADDQ.b  #1,numexp(%a3)        |; increment number exponent count
LAB_299C:
    MOVE.l  #0x98967F70,FAC2_m(%a3)    |; 9999999.4375 mantissa
    MOVE.w  #0x9800,FAC2_e(%a3)        |; 9999999.4375 exponent & sign
    ...
    BHI     LAB_29B9                        |; go do /10 if FAC1 > 9999999.4375

    BEQ     LAB_29C3                        |; branch if FAC1 = 9999999.4375

                            |; FAC1 < 9999999.4375
    MOVE.l  #0xF423F800,FAC2_m(%a3)    |; set mantissa for 999999.5
    MOVE.w  #0x9400,FAC2_e(%a3)        |; set exponent for 999999.5
...
LAB_29A7:
    BSR     LAB_27F0                        |; fast compare FAC1 with FAC2
                            ...
                            |; returns d0= 0 C=0 if FAC1 = FAC2
                            |; returns d0=-1 C=1 if FAC1 < FAC2
    BHI     LAB_29C0                        |; branch if FAC1 > 99999.9375,no decimal places
    ...
    BCC     LAB_29B7                        |; branch if no carry
...
LAB_29B7:
    ...
    BRA     LAB_29A7                        |; go test again

                            |; now we have just the digits to do
LAB_29C0:
    MOVE.l  #0x80000000,FAC2_m(%a3)    |; set mantissa for 0.5
    MOVE.w  #0x8000,FAC2_e(%a3)        |; set exponent for 0.5
...
LAB_29C3:
    BSR     LAB_2831                        |; convert FAC1 floating to fixed
                            |; result in %d0 and Itemp
    MOVE.L  #0x01,%d2                        |; set default digits before dp = 1
    MOVE.b  numexp(%a3),%d0        |; get number exponent count
    ADD.b   #8,%d0                |; allow 7 digits before point
    BMI     LAB_29D9                        |; if -ve then 1 digit before dp

    CMP.b   #0x09,%d0                        |; d0>=9 if n>=1E7
    BCC     LAB_29D9                        |; branch if >= 0x09

                            |; < 0x08
    SUBQ.b  #1,%d0                |; take 1 from digit count
    MOVE.b  %d0,%d2                |; copy byte
    MOVE.L  #0x02,%d0                        |; set exponent adjust
LAB_29D9:
    MOVE.L  #0,%d1                |; set output string index
    SUBQ.b  #2,%d0                |; -2
...
LAB_29E4:
    ...
    BEQ     LAB_29F7                        |; branch if no digits before dp

    ADDQ.l  #1,%d1                |; increment index
    MOVE.b  #'0',(%a1,%d1.w)        |; save to output string
LAB_29F7:
    MOVE.L  #0,%d2                |; clear index (point to 1,000,000)
    move.l  #0x80-0x100,%d0        |; set output character
LAB_29FB:
...
LAB_29FD:
    ...
    BCS     LAB_2A18                        |; did carry so has wrapped past zero

    BEQ     LAB_29FD                        |; no wrap and +ve test so try again

    BRA     LAB_2A1A                        |; found this digit

LAB_2A18:
    BNE     LAB_29FD                        |; wrap and -ve test so try again

LAB_2A1A:
    BCC     LAB_2A21                        |; branch if +ve test result
...
LAB_2A74:
    ...
    BRA     LAB_2A91                        |; go set string pointer (%a0) and exit
...
LAB_27F0:
    MOVE.L  #0,%d0                |; set for FAC1 = FAC2
    MOVE.b  FAC2_e(%a3),%d1        |; get FAC2 exponent
    CMP.b   FAC1_e(%a3),%d1        |; compare FAC1 exponent with FAC2 exponent
    BNE     LAB_27F1                        |; branch if different

    MOVE.l  FAC2_m(%a3),%d1        |; get FAC2 mantissa
    CMP.l   FAC1_m(%a3),%d1        |; compare mantissas
    BEQ     LAB_27F3                        |; exit if mantissas equal

LAB_27F1:
    BCS     LAB_27F2                        |; if FAC1 > FAC2 return d0=+1,C=0
...
LAB_POON:
    MOVE.l  #0x80000000,FAC1_m(%a3)    |; 1 mantissa
    MOVE.w  #0x8100,FAC1_e(%a3)        |; 1 exonent & sign
    RTS     
/***********************************************************************************/
#
|; make FAC1 = 0
...
LAB_POWER:
    ...
    BEQ     LAB_POON                        |; if zero go return 1

    TST.b   FAC2_e(%a3)                        |; test number
    BEQ     LAB_POZE                        |; if zero go return 0

    MOVE.b  FAC2_s(%a3),-(%sp)        |; save number sign
    BPL     LAB_POWP                        |; power of positive number
    ...
    BCS     LAB_POWP                        |; if integer value odd then leave result -ve
...
LAB_GTHAN:
    ...
    EORI.b  #0x80,FAC1_s(%a3)        |; (else) toggle FAC1 sign bit
RTS_020:
    RTS     
/***********************************************************************************/
#
...
LAB_EX1:
    MOVE.l  #0x80000000,FAC1_m(%a3)    |; +1 mantissa
    MOVE.w  #0x8100,FAC1_e(%a3)        |; +1 sign & exponent
    RTS     |; do over/under flow
LAB_EXOU:
    TST.b   FAC1_s(%a3)                        |; test sign
    ...
    RTS     |; fraction was zero so do 2^n
LAB_EXOF:
    MOVE.l  #0x80000000,FAC1_m(%a3)    |; +n mantissa
    MOVE.b  #0,FAC1_s(%a3)        |; clear sign
    TST.b   cosout(%a3)                        |; test sign flag
    BPL     LAB_EXOL                        |; branch if +ve
...
LAB_EXP:
    MOVE.b  FAC1_e(%a3),%d0        |; get exponent
    BEQ     LAB_EX1                        |; return 1 for zero in

    CMP.b   #0x64,%d0                        |; compare exponent with min
    BCS     LAB_EX1                        |; if smaller just return 1
...
|;*    MOVEM.l    %d1-%d6/%a0,-(%sp)        |; save the registers
    ...
    BHI     LAB_EXOU                        |; go do over/under flow if greater

    BNE     LAB_EXCM                        |; branch if less

                            |; else is 2^7
    CMP.l   #0xB00F33C7,%d1        |; compare mantissa with n*2^7 max
    BCC     LAB_EXOU                        |; if => go over/underflow

LAB_EXCM:
    TST.b   FAC1_s(%a3)                        |; test sign
    BPL     LAB_EXPS                        |; branch if arg +ve

    MOVE.b  #0xFF,cosout(%a3)        |; flag -ve number
    MOVE.b  #0,FAC1_s(%a3)        |; take absolute value
LAB_EXPS:
                            |; now do n/LOG(2)
    MOVE.l  #0xB8AA3B29,FAC2_m(%a3)    |; 1/LOG(2) mantissa
    MOVE.w  #0x8100,FAC2_e(%a3)        |; 1/LOG(2) exponent & sign
    ...
    BLS     LAB_ESML                        |; branch if < 1 (d5 is 0 or -ve)
    ...
    MOVE.w  #0x8000,FAC1_e(%a3)        |; set exponent & sign
...
|; %d5 = loop count
    ...
    MOVE.L  #0,%d4                |; clear shift count
 
                            |; cordic loop, shifts 4 and 13 (and 39
                            |; if it went that far) need to be repeated
    MOVE.L  #3,%d5                |; 4 loops
    BSR     LAB_EXCC                        |; do loops 1 through 4
    SUBQ.w  #4,%a0                |; do table entry again
    SUBQ.l  #1,%d4                |; do shift count again
    MOVE.L  #9,%d5                |; 10 loops
    BSR     LAB_EXCC                        |; do loops 4 (again) through 13
    SUBQ.w  #4,%a0                |; do table entry again
    SUBQ.l  #1,%d4                |; do shift count again
    MOVE.L  #18,%d5                        |; 19 loops
    BSR     LAB_EXCC                        |; do loops 13 (again) through 31
 
                            |; now get the result
    TST.b   cosout(%a3)                        |; test sign flag
    BPL     LAB_EXPL                        |; branch if +ve

    NEG.l   %d1                |; do -y
    NEG.b   tpower(%a3)                        |; do -exp
LAB_EXPL:
    move.l  #0x83-0x100,%d0        |; set exponent
    ADD.l   %d1,%d6                |; y = y +/- x
    BMI     LAB_EXRN                        |; branch if result normal

LAB_EXNN:
    SUBQ.l  #1,%d0                |; decrement exponent
    ADD.l   %d6,%d6                |; shift mantissa
    BPL     LAB_EXNN                        |; loop if not normal
...
NextPRN:
    move.l  #0xAF-0x100,%d1        |; set EOR value
...
Ninc1:
    ...
    MOVE.w  #0x8000,FAC1_e(%a3)        |; set the exponent and clear the sign
...
LAB_TAN:
    BSR     LAB_SIN                        |; go do SIN/COS cordic compute
    MOVE.w  FAC1_e(%a3),FAC2_e(%a3)    |; copy exponent & sign from FAC1 to FAC2
    MOVE.l  FAC1_m(%a3),FAC2_m(%a3)    |; copy FAC1 mantissa to FAC2 mantissa
...
LAB_COS:
    MOVE.l  #0xC90FDAA3,FAC2_m(%a3)    |; pi/2 mantissa (LSB is rounded up so
                            |; COS(PI/2)=0)
    MOVE.w  #0x8100,FAC2_e(%a3)        |; pi/2 exponent and sign
...
LAB_SIN:
    ...
    MOVE.l  #0xA2F9836F,FAC2_m(%a3)    |; 1/pi mantissa (LSB is rounded up so SIN(PI)=0)
    MOVE.w  #0x7F00,FAC2_e(%a3)        |; 1/pi exponent & sign
    MOVE.b  FAC1_s(%a3),FAC_sc(%a3)    |; sign = FAC1 sign (b7)
    ...
    BNE     LAB_CORD                        |; if fraction go test quadrant and adjust

                            |; else no fraction so do zero
LAB_SCZE:
    move.l  #0x81-0x100,%d2        |; set exponent for 1.0
    MOVE.L  #0,%d3                |; set exponent for 0.0
...
LAB_CORD:
    ...
    BCC     LAB_LTPF                        |; branch if less than 0.5

    EORI.b  #0xFF,FAC1_s(%a3)        |; toggle result sign
LAB_LTPF:
    ADD.l   %d6,%d6                |; shift 0.25 bit into carry
    BCC     LAB_LTPT                        |; branch if less than 0.25

    EORI.b  #0xFF,cosout(%a3)        |; toggle needed result
    EORI.b  #0xFF,FAC_sc(%a3)        |; toggle sign compare for TAN
...
mainloop:
    ...
    BPL     subloop                        |; go do subtract if > 1
...
nexta:
    ...
    BNE     mainloop                        |; loop if not all done

                            |; now untangle output value
    move.l  #0x81-0x100,%d2        |; set exponent for 0 to .99 rec.
...
LAB_ATN:
    ...
    BCS     LAB_ATLE                        |; branch if n<1

    BNE     LAB_ATGO                        |; branch if n>1

    MOVE.l  FAC1_m(%a3),%d0        |; get mantissa
    ADD.l   %d0,%d0                |; shift left
    BEQ     LAB_ATLE                        |; branch if n=1
...
LAB_ATGO:
    ...
    MOVE.b  #0xFF,cosout(%a3)        |; set inverse result needed
LAB_ATLE:
    MOVE.l  FAC1_m(%a3),%d0        |; get FAC1 mantissa
    ...
    MOVE.L  #0,%d6                |; Z = 0
    MOVE.l  #1<<30,%d1                        |; y = 1
    MOVE.L  #29,%d5                        |; loop 30 times
    MOVE.L  #1,%d4                |; shift counter
    BRA     LAB_ATCD                        |; enter loop
...
LAB_CATN:
    ...
    MOVE.b  #0x82,FAC1_e(%a3)        |; set new exponent
    ...
    MOVE.b  #0,FAC1_s(%a3)        |; clear sign
    MOVE.l  #0xC90FDAA2,FAC2_m(%a3)    |; set -(pi/2)
    MOVE.w  #0x8180,FAC2_e(%a3)        |; set exponent and sign
    MOVE.b  #0xFF,FAC_sc(%a3)        |; set sign compare
...
LAB_BTST:
    ...
    MOVE.L  #0,%d0                          |; set the result as zero
    BTST    %d1,(%a0)                       |; test bit
    BEQ     LAB_27DB                        |; branch if zero (already correct)

    MOVE.L  #-1,%d0                         |; set for -1 result
...
|; perform USING$()
    ...
    .equ    fsti, 4                    |;  4(%sp) format string this index
    .equ    fsli, 6                    |;  6(%sp) format string last index
    .equ    fsdpi, 8                    |;  8(%sp) format string decimal point index
    .equ    fsdc, 10                    |; 10(%sp) format string decimal characters
    .equ    fend, 12-4                    |;  x(%sp) end-4, fsd is popped by itself

    .equ    ofchr, '#'                    |; the overflow character

LAB_USINGS:
    TST.b   Dtypef(%a3)                        |; test data type, 0x80=string
...
|; make a null return string for the first string add
    ...
    MOVE.L  #0,%d1                |; make a null string
...
LAB_U002:
    ...
    MOVE.b  #0x00,FAC_sc(%a3)        |; clear the sign compare
...
|; process the number string, length in %d6, decimal point index in d2
    ...
    MOVE.L  #0,%d6                |; clear the number string index
    MOVE.L  #'.',%d4                        |; set the decimal point character
LAB_U005:
...
LAB_U006:
    ...
    BEQ     LAB_U010                        |; if null then number complete

    CMP.b   #'E',%d0                        |; compare the character with an "E"
    BEQ     LAB_U008                        |; was sx[.x]Esxx so go handle sci notation

    CMP.b   %d4,%d0                |; compare the character with "."
    BNE     LAB_U006                        |; if not decimal point go get the next digit

    BRA     LAB_U005                        |; go save the index and get the next digit
...
|; found an sx[.x]E-xx number so check the exponent magnitude
    ...
    BEQ     LAB_U009                        |; if it was sx[.x]E-0x go get the exponent
                            |; 1s character

    MOVE.L  #10,%d0                        |; else start writing at index 10
    BRA     LAB_U00A                        |; go copy the digits

|; found an sx[.x]E-0x number so get the exponent magnitude

LAB_U009:
    ADDQ.w  #1,%d6                |; increment the index to the exponent 1s
    MOVE.L  #0x0F,%d0                        |; set the mask for the exponent 1s digit
...
LAB_U00A:
    ...
    BNE     LAB_U00B                        |; if it is sx.xE-0x skip the increment
...
LAB_U00B:
    ...
    MOVE.L  #10,%d0                        |; set the maximum write index
    SUB.w   %d2,%d0                |; compare the index with the maximum
    BGT     LAB_U00C                        |; if the index < the maximum continue
    ...
    BGT     LAB_U00C                        |; if > 1 continue

    MOVE.L  #0,%d3                |; else allow for the decimal point
LAB_U00C:
    MOVE.w  %d2,%d6                |; copy the write index as the number
                            |; string length
    MOVE.L  #0,%d0                |; clear %d0 to null terminate the number
                            |; string
LAB_U00D:
...
LAB_U018:
    ...
    BLS     LAB_U022                        |; if done the fraction digits go do integer

    MOVE.b  (%a0,%d4.w),%d0        |; get a new format string character
    CMP.b   #'%',%d0                        |; compare it with "%"
    BEQ     LAB_U01C                        |; if "%" go copy a number character

    CMP.b   #'#',%d0                        |; compare it with "#"
    BNE     LAB_U018                        |; if not "#" go do the next new format character

LAB_U01C:
    MOVE.L  #'0',%d0                        |; default to "0" character
    CMP.w   %d3,%d6                |; compare the number string index with length
    BLS     LAB_U020                        |; if there skip the character get
...
LAB_U020:
    MOVE.b  %d0,(%a0,%d4.w)        |; save the number character to the new format
                            |; string
    BRA     LAB_U018                        |; go do the next new format character

|; now copy the integer digit characters from the number string

LAB_U022:
    MOVE.L  #0,%d6                |; clear the sign done flag
    MOVE.L  #0,%d5                |; clear the sign present flag
    SUBQ.w  #1,%d2                |; decrement the number string index
    BNE     LAB_U026                        |; if not now at sign continue

    MOVE.L  #1,%d2                |; increment the number string index
    MOVE.b  #'0',(%a2,%d2.w)        |; replace the point with a zero
LAB_U026:
    MOVE.w  fsdpi(%sp),%d4        |; get the new format string decimal point index
    CMP.w   %d4,%d1                |; compare it with the new format string length
    BCC     LAB_U02A                        |; if within the string go use the index

    MOVE.w  %d1,%d4                |; else set the index to the end of the string
LAB_U02A:
    SUBQ.w  #1,%d4                |; decrement the new format string index
    BMI     LAB_U03E                        |; if all done go test for any overflow

    MOVE.b  (%a0,%d4.w),%d0        |; else get a new format string character

    MOVE.L  #'0',%d7                        |; default to "0" character
    CMP.b   #'%',%d0                        |; compare it with "%"
    BEQ     LAB_U02B                        |; if "%" go copy a number character

    MOVE.L  #' ',%d7                        |; default to " " character
    CMP.b   #'#',%d0                        |; compare it with "#"
    BNE     LAB_U02C                        |; if not "#" go try ","

LAB_U02B:
    TST.w   %d2                |; test the number string index
    BNE     LAB_U036                        |; if not at the sign go get a number character

    BRA     LAB_U03C                        |; else go save the default character

LAB_U02C:
    CMP.b   #',',%d0                        |; compare it with ","
    BNE     LAB_U030                        |; if not "," go try the sign characters

    TST.w   %d2                |; test the number string index
    BNE     LAB_U02E                        |; if not at the sign keep the ","

    CMP.b   #'%',-1(%a0,%d4.w)        |; else compare the next format string character
                            |; with "%"
    BNE     LAB_U03C                        |; if not "%" keep the default character

LAB_U02E:
    MOVE.b  %d0,%d7                |; else use the "," character
    BRA     LAB_U03C                        |; go save the character to the string

LAB_U030:
    CMP.b   #'-',%d0                        |; compare it with "-"
    BEQ     LAB_U034                        |; if "-" go do the sign character

    CMP.b   #'+',%d0                        |; compare it with "+"
    BNE     LAB_U02A                        |; if not "+" go do the next new format character

    CMP.b   #'-',(%a2)                        |; compare the sign character with "-"
    BEQ     LAB_U034                        |; if "-" don't change the sign character

    MOVE.b  #'+',(%a2)                        |; else make the sign character "+"
LAB_U034:
    MOVE.b  %d0,%d5                |; set the sign present flag
    TST.w   %d2                |; test the number string index
    BEQ     LAB_U038                        |; if at the sign keep the default character

LAB_U036:
    MOVE.b  (%a2,%d2.w),%d7        |; else get a character from the number string
    SUBQ.w  #1,%d2                |; decrement the number string index
    BRA     LAB_U03C                        |; go save the character

LAB_U038:
    TST.b   %d6                |; test the sign done flag
    BNE     LAB_U03C                        |; if the sign has been done go use the space
                            |; character
...
LAB_U03C:
    MOVE.b  %d7,(%a0,%d4.w)        |; save the number character to the new format
                            |; string
    BRA     LAB_U02A                        |; go do the next new format character

|; test for overflow conditions

LAB_U03E:
    TST.w   %d2                |; test the number string index
    BNE     LAB_U040                        |; if all the digits aren't done go output
                            |; an overflow indication

|; test for sign overflows

    TST.b   %d5                |; test the sign present flag
    BEQ     LAB_U04A                        |; if no sign present go add the string

|; there was a sign in the format string

    TST.b   %d6                |; test the sign done flag
    BNE     LAB_U04A                        |; if the sign is done go add the string

|; the sign isn't done so see if it was mandatory

    CMPI.b  #'+',%d5                        |; compare the sign with "+"
    BEQ     LAB_U040                        |; if it was "+" go output an overflow
                            |; indication

|; the sign wasn't mandatory but the number may have been negative

    CMP.b   #'-',(%a2)                        |; compare the sign character with "-"
    BNE     LAB_U04A                        |; if it wasn't "-" go add the string
...
LAB_U040:
    MOVE.L  #ofchr,%d5                        |; set the overflow character
    ...
    BGT     LAB_U044                        |; if not zero continue
...
LAB_U044:
    ...
    BEQ     LAB_U046                        |; if "#" go use the overflow character
    ...
    CMPI.b  #'.',%d0                        |; compare it with "." special format character
    BNE     LAB_U048                        |; if not "." skip the using overflow character
...
|; now check for any tail on the format string
    ...
    BEQ     LAB_U04C                        |; if at start of string skip the output

    MOVE.w  %d0,fsli(%sp)                        |; save this index to the last index
    BSR     LAB_ProcFo                        |; now process the format string
    TST.b   %d2                |; test the special characters flag
    BNE     LAB_U04C                        |; if special characters present skip the output
...
LAB_DupFmt:
    ...
    BHI     LAB_D002                        |; if > 0 skip the correction
...
|; copy the characters from the format string
    ...
    MOVE.L  #0,%d4                |; clear the new string index
LAB_D00A:
    ...
    BNE     LAB_D00E                        |; if not there skip the reset

    MOVE.L  #0,%d2                |; else reset the format string index
LAB_D00E:
    CMP.w   %d2,%d6                |; compare the index with this index
    BNE     LAB_D00A                        |; if not equal go do the next character
...
###    MOVE.w    #-1,4+fsdpi(%sp)        |; set the format string decimal point index
    MOVE.L  #0,%d5                |; no decimal point
    MOVE.L  #0,%d3                |; no decimal characters
    MOVE.L  #0,%d2                |; no special characters
LAB_P004:
    MOVE.b  (%a1,%d6.w),%d0        |; get a format string byte

    CMP.b   #',',%d0                        |; compare it with ","
    BEQ     LAB_P01A                        |; if "," go do the next format string byte

    CMP.b   #'#',%d0                        |; compare it with "#"
    BEQ     LAB_P008                        |; if "#" go flag special characters

    CMP.b   #'%',%d0                        |; compare it with "%"
    BNE     LAB_P00C                        |; if not "%" go try "+"

LAB_P008:
    TST.l   %d5                |; test the decimal point flag
    BPL     LAB_P00E                        |; if no point skip counting decimal characters

    ADDQ.w  #1,%d3                |; else increment the decimal character count
    BRA     LAB_P01A                        |; go do the next character

LAB_P00C:
    CMP.b   #'+',%d0                        |; compare it with "+"
    BEQ     LAB_P00E                        |; if "+" go flag special characters

    CMP.b   #'-',%d0                        |; compare it with "-"
    BNE     LAB_P010                        |; if not "-" go check decimal point
...
LAB_P010:
    CMP.b   #'.',%d0                        |; compare it with "."
    BNE     LAB_P018                        |; if not "." go check next

|; "." a decimal point

    TST.l   %d5                |; if there is already a decimal point
    BMI     LAB_P01A                        |; go do the next character
    ...
    MOVE.L  #-1,%d5                        |; flag decimal point
    OR.b        %d0,%d2                |; flag special characters
    BRA     LAB_P01A                        |; go do the next character
...
LAB_P018:
    ...
    BNE     LAB_P01E                        |; if so exit the format string process

LAB_P01A:
    ADDQ.w  #1,%d6                |; increment the format string index
    CMP.w   %d6,%d7                |; compare it with the format string length
    BHI     LAB_P004                        |; if length > index go get the next character

    MOVE.L  #0,%d6                |; length = index so reset the format string
                            |; index
LAB_P01E:
...
LAB_BINS:
    ...
    MOVE.L  #0x1F,%d2                        |; bit count-1
    LEA     Binss(%a3),%a0        |; point to string
    MOVE.L  #0x30,%d4                        |; "0" character for ADDX
NextB1:
    MOVE.L  #0,%d3                |; clear byte
...
LAB_HEXS:
    ...
    MOVE.L  #0x07,%d2                        |; nibble count-1
    LEA     Hexss(%a3),%a0        |; point to string
    MOVE.L  #0x30,%d4                        |; "0" character for ABCD
NextH1:
...
LAB_FB95:
    MOVE.b  #0x00,ccnull(%a3)        |; clear timer because we got a byte
...
LAB_MAXN:
    BSR     LAB_PHFA                        |; push FAC1, evaluate expression,
                            |; pull FAC2 & compare with FAC1
    BCC     LAB_MAXN                        |; branch if no swap to do

    BSR     LAB_279B                        |; copy FAC2 to FAC1
    BRA     LAB_MAXN                        |; go do next
...
LAB_MINN:
    BSR     LAB_PHFA                        |; push FAC1, evaluate expression,
                            |; pull FAC2 & compare with FAC1
    BLS     LAB_MINN                        |; branch if no swap to do

    BSR     LAB_279B                        |; copy FAC2 to FAC1
    BRA     LAB_MINN                        |; go do next (branch always)
...
|; tab size greater than the line length.
    ...
    BCC     LAB_NSTT                        |; branch if >= tab size
...
WExit:
    ...
    BEQ     LAB_WDLP                        |; branch if infinite line

    CMP.b   TabSiz(%a3),%d0        |; compare with tab size
    BCC     LAB_WDLP                        |; branch if >= tab size

    MOVE.b  %d0,TabSiz(%a3)        |; else make tab size = terminal width
LAB_WDLP:
    SUB.b   TabSiz(%a3),%d0        |; subtract tab size
    BCC     LAB_WDLP                        |; loop while no borrow
...
LAB_SQR:
    ...
    MOVE.L  #0,%d2                |; clear remainder
    MOVE.l  %d2,%d1                |; clear root

    MOVE.L  #0x1F,%d3                        |; 0x1F for DBF, 64 pairs of bits to
...
LAB_SQE2:
    ...
    BCS     LAB_SQNS                        |; skip sub if remainder smaller
...
LAB_SQNS:
    ...
    LSR.w   #1,%d0                |; /2
    BCC     LAB_SQNA                        |; skip increment if carry clear
...
LAB_PI:
    MOVE.l  #0xC90FDAA2,FAC1_m(%a3)    |; pi mantissa (32 bit)
    MOVE.w  #0x8200,FAC1_e(%a3)        |; pi exponent and sign
    RTS     
/***********************************************************************************/
#
|; perform TWOPI
...
LAB_TWOPI:
    MOVE.l  #0xC90FDAA2,FAC1_m(%a3)    |; 2pi mantissa (32 bit)
    MOVE.w  #0x8300,FAC1_e(%a3)        |; 2pi exponent and sign
    RTS     
/***********************************************************************************/
#
...
LAB_2X05:
    ...
    BRA     LAB_2Y01                        |; all done, go evaluate

LAB_2902:
    CMP.b   #'.',%d0                        |; else compare with '.'
    BEQ     LAB_2904                        |; branch if was '.'
...
LAB_CHX1:
    ...
    BSR     LAB_IGBY                        |; get next character
    BCS     LAB_CHX1                        |; loop while numeric character
...
LAB_CHX3:
    ...
    BMI     LAB_exxf                        |; branch if already mormalised
...
LAB_CBXN:
    BSR     LAB_IGBY                        |; increment & scan memory
    BCC     LAB_CHX3                        |; if not numeric character go return value

    CMP.b   #'2',%d0                        |; compare with "2" (max+1)
    BCC     LAB_CHX3                        |; if >="2" go return value

    MOVE.l  %d1,%d2                |; copy value
    BSR     d1x02                |; multiply %d1 by 2 and add character
    BCC     LAB_CBXN                        |; loop for more if no overflow
...
LAB_CBX1:
    ...
    CMP.b   #'2',%d0                        |; compare with "2" (max+1)
    BCS     LAB_CBX1                        |; loop if <"2"

    BRA     LAB_CHX3                        |; if not numeric character go return value
...
d1x16:
    ...
    BCS     RTS_024                        |; return if overflow
...
|; now add in new digit
    ...
    BCS     RTS_024                        |; return if overflow, it should never ever do
                            |; this

    MOVE.l  %d2,%d1                |; copy result
    RTS     |; half way decent times 10 with overflow checks

d1x10:
    MOVE.l  %d1,%d2                |; copy value
    ADD.l   %d2,%d2                |; times two
    BCS     RTS_025                        |; return if overflow
...
|;    bsr     LAB_24D5                        |; normalize FAC1
|;    bsr     LAB_27BA                        |; round FAC1
    move.l  %a3@(FAC1_m),%d0                |; get mantissa
    ror.l   #8,%d0                          |; mask off high 9 bits
    swap    %d0                             |; 
    andi.w  #0x7f,%d0                       |; 
    swap    %d0                             |; 
    move.l  %d0,%a3@(FAC1_FPU)              |; copy to output
    eor.l   %d0,%d0                         |; clear working register
    move.w  %a3@(FAC1_e),%d0                |; get sign & exponent
    ror.w   #8,%d0                          |; get exponent in byte position
    subq.b  #2,%d0                          |; correct exponent offset
    ror.w   #8,%d0                          |; revert byte order
    lsr.b   #7,%d0                          |; fix sign position
    ror.w   #1,%d0                          |; move sign & exponent to position
    swap    %d0                             |; 
    or.l    %d0,%a3@(FAC1_FPU)              |; save to output
    fmove.s %a3@(FAC1_FPU),%fp1             |; copy to FPU
...
|;    bsr     LAB_27BA                        |; round FAC1
    ...
    andi.w  #0x7f,%d0                       |; 
    swap    %d0                             |; 
    move.l  %d0,%a3@(FAC2_FPU)              |; copy to output
    eor.l   %d0,%d0                         |; clear working register
    ...
    ror.w   #1,%d0                          |; move sign & exponent to position
    swap    %d0                             |; 
    or.l    %d0,%a3@(FAC2_FPU)              |; save to output
    fmove.s %a3@(FAC2_FPU),%fp2             |; copy to FPU
    move.l  %sp@+,%d0                       |; restore working register
    rts                                     |; 

FPUtoFAC1:
    move.l  %d0,%sp@-                       |; save working register
    fmove.s %fp1,%a3@(FAC1_FPU)             |; get value from FPU
    move.l  %a3@(FAC1_FPU),%d0              |; get value in working register
    swap    %d0                             |; mask off high sign & exponent
    andi.w  #0x7f,%d0                       |; 
    ori.b   #0x80,%d0                       |; set high bit for some reason
    ror.l   #8,%d0                          |; shift mantissa to 32 bits
    move.l  %d0,%a3@(FAC1_m)                |; save mantissa
    move.l  %a3@(FAC1_FPU),%d0              |; get value again
    swap    %d0                             |; get sign & exponent
    andi.w  #0xff80,%d0                     |; mask sign & exponent
    rol.w   #8,%d0                          |; get exponent in low byte
    rol.w   #1,%d0                          |;
    addq.b  #2,%d0                          |; correct exponent offset
    ror.w   #8,%d0                          |; swap byte order for saving
    lsl.b   #7,%d0                          |; shift sign to high bit
    move.w  %d0,%a3@(FAC1_e)                |; save sign & exponent
    move.l  %sp@+,%d0                       |; restore working register
    rts                                     |;

FPUtoFAC2:
    move.l  %d0,%sp@-                       |; save working register
    fmove.s %fp2,%a3@(FAC2_FPU)             |; get value from FPU
    move.l  %a3@(FAC2_FPU),%d0              |; get value in working register
    swap    %d0                             |; mask off high sign & exponent
    andi.w  #0x7f,%d0                       |; 
    ori.b   #0x80,%d0                       |; set high bit for some reason
    ror.l   #8,%d0                          |; shift mantissa to 32 bits
    move.l  %d0,%a3@(FAC2_m)                |; save mantissa
    move.l  %a3@(FAC2_FPU),%d0              |; get value again
    swap    %d0                             |; get sign & exponent
    andi.w  #0xff80,%d0                     |; mask sign & exponent
    rol.w   #8,%d0                          |; get exponent in low byte
    rol.w   #1,%d0                          |;
    ...
    ror.w   #8,%d0                          |; swap byte order for saving
    lsl.b   #7,%d0                          |; shift sign to high bit
    move.w  %d0,%a3@(FAC2_e)                |; save sign & exponent
    move.l  %sp@+,%d0                       |; restore working register
    rts                                     |;

LAB_FADD:
    bsr     Fac1toFPU                       |; get FAC1
    bsr     Fac2toFPU                       |; get FAC2
    fadd.x  %fp2,%fp1                       |; do add
    bra     FPUtoFAC1                       |; move sum to FAC1 & return

LAB_FSUB:
    bsr     Fac1toFPU                       |; get FAC1
    bsr     Fac2toFPU                       |; get FAC2
    fsub.x  %fp1,%fp2                       |; do sub
    fmove.x %fp2,%fp1                       |; swap registers
    bra     FPUtoFAC1                       |; move sum to FAC1 & return

LAB_FMUL:
    bsr     Fac1toFPU                       |; get FAC1
    bsr     Fac2toFPU                       |; get FAC2
    fmul.x  %fp2,%fp1                       |; do add
    bra     FPUtoFAC1                       |; move sum to FAC1 & return

LAB_FDIV:
    bsr     Fac1toFPU                       |; get FAC1
    bsr     Fac2toFPU                       |; get FAC2
    fdiv.x  %fp1,%fp2                       |; do sub
    fmove.x %fp2,%fp1                       |; swap registers
    bra     FPUtoFAC1                       |; move sum to FAC1 & return
...
|; token values needed for BASIC
    ...
    .equ    TK_FADD,    TK_MINUS+1
    .equ    TK_FSUB,    TK_FADD+1
    .equ    TK_MULT,    TK_FSUB+1           |; 0xB5
    .equ    TK_DIV,     TK_MULT+1           |; 0xB6
    .equ    TK_FMUL,    TK_DIV+1
    .equ    TK_FDIV,    TK_FMUL+1
    .equ    TK_POWER,   TK_FDIV+1           |; 0xB7
...
|; constants are normalised to n integer bits and are tanh(2^-i)
    .equ    n, 2
TAB_HTHET:
    dc.l    0x2327d4f4                        |; atnh(2^-1) .549306144
    dc.l    0x1058aefa                        |; atnh(2^-2) .255412812
    dc.l    0x080ac48e                        |; atnh(2^-3)
    dc.l    0x04015622                        |; atnh(2^-4)
    dc.l    0x02002ab0                        |; atnh(2^-5)
    dc.l    0x01000554                        |; atnh(2^-6)
    dc.l    0x008000aa                        |; atnh(2^-7)
    dc.l    0x00400014                        |; atnh(2^-8)
    dc.l    0x00200002                        |; atnh(2^-9)
    dc.l    0x00100000                        |; atnh(2^-10)
    dc.l    0x00080000                        |; atnh(2^-11)
    dc.l    0x00040000                        |; atnh(2^-12)
    dc.l    0x00020000                        |; atnh(2^-13)
    dc.l    0x00010000                        |; atnh(2^-14)
    dc.l    0x00008000                        |; atnh(2^-15)
    dc.l    0x00004000                        |; atnh(2^-16)
    dc.l    0x00002000                        |; atnh(2^-17)
    dc.l    0x00001000                        |; atnh(2^-18)
    dc.l    0x00000800                        |; atnh(2^-19)
    dc.l    0x00000400                        |; atnh(2^-20)
    dc.l    0x00000200                        |; atnh(2^-21)
    dc.l    0x00000100                        |; atnh(2^-22)
    dc.l    0x00000080                        |; atnh(2^-23)
    dc.l    0x00000040                        |; atnh(2^-24)
    dc.l    0x00000020                        |; atnh(2^-25)
    dc.l    0x00000010                        |; atnh(2^-26)
    dc.l    0x00000008                        |; atnh(2^-27)
    dc.l    0x00000004                        |; atnh(2^-28)
    dc.l    0x00000002                        |; atnh(2^-29)
    dc.l    0x00000001                        |; atnh(2^-30)
    dc.l    0x00000000                        |; atnh(2^-31)
    dc.l    0x00000000                        |; atnh(2^-32)
    .equ    KFCTSEED, 0x26A3D110        |; 0x26A3D110
...
LAB_OPPT:
    ...
    dc.w    0x0079                          |; FADD (FPU Add)
    dc.w    LAB_FADD-LAB_OPPT

    dc.w    0x0079                          |; FSUB (FPU Subtract)
    dc.w    LAB_FSUB-LAB_OPPT
    ...
    dc.w    0x007B                          |; FMUL (FPU Multiply)
    dc.w    LAB_FMUL-LAB_OPPT

    dc.w    0x007B                          |; FDIV (FPU Divide)
    dc.w    LAB_FDIV-LAB_OPPT
...
LAB_KEYT:
    ...
    dc.b    'F',2
    dc.w    KEY_FADD-TAB_STAR               |; FADD
    dc.b    'F',2
    dc.w    KEY_FSUB-TAB_STAR               |; FSUB
    ...
    dc.b    'F',2
    dc.w    KEY_FMUL-TAB_STAR               |; FMUL
    dc.b    'F',2
    dc.w    KEY_FDIV-TAB_STAR               |; FDIV
...
LAB_BAER:
    ...
    dc.w    LAB_NI-LAB_BAER                        |; 0x2E not implemented

LAB_NF:  .ascii  "NEXT without FOR\0"
LAB_SN:  .ascii  "Syntax\0"
LAB_RG:  .ascii  "RETURN without GOSUB\0"
LAB_OD:  .ascii  "Out of DATA\0"
LAB_FC:  .ascii  "Function call\0"
LAB_OV:  .ascii  "Overflow\0"
LAB_OM:  .ascii  "Out of memory\0"
LAB_US:  .ascii  "Undefined statement\0"
LAB_BS:  .ascii  "Array bounds\0"
LAB_DD:  .ascii  "Double dimension\0"
LAB_D0:  .ascii  "Divide by zero\0"
LAB_ID:  .ascii  "Illegal direct\0"
LAB_TM:  .ascii  "Type mismatch\0"
LAB_LS:  .ascii  "String too long\0"
LAB_ST:  .ascii  "String too complex\0"
LAB_CN:  .ascii  "Can't continue\0"
LAB_UF:  .ascii  "Undefined function\0"
LAB_LD:  .ascii  "LOOP without DO\0"
LAB_UV:  .ascii  "Undefined variable\0"
LAB_UA:  .ascii  "Undimensioned array\0"
LAB_WD:  .ascii  "Wrong dimensions\0"
LAB_AD:  .ascii  "Address\0"
LAB_FO:  .ascii  "Format\0"
LAB_NI:  .ascii  "Not implemented\0"
...
LAB_SMSG:
    .ascii  " Bytes free\r\n"
    .ascii  "Enhanced 68k BASIC Version 3.54\r\n\0"
