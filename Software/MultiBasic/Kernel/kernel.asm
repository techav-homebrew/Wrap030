...
.ifndef     SIMULATE
    ...
    move.l  #MAXPROCS-1,%d0                 |; get number of users
1:
...
kInit:
    ...
    move.l  #MAXPROCS-1,%d0                 |; start at end of user numbers
    lea     PROCTABLE,%a0                   |; pointer to user table
1:
...
1:
    ...
    move.l  %a2@(%d2.L),%a1@(utblConInDev)  |;
    lea     tblUserConOut,%a2               |; set user console out device address
    move.l  %a2@(%d2.L),%a1@(utblConOutDev) |;
...
SaveUserContext:
    ...
    |;debugPrintStrI  "S"
    lea     PROCTABLE,%a0                   |; get pointer to user table
    move.l  PROCNUM,%d0                     |; get current user number
...
NextUser:
    ...
    cmp.l   #MAXPROCS,%d0                   |; is this the last user?
    blt.s   1f
    moveq.l #0,%d0                          |; loop back around to first user
1:
    move.l  %d0,PROCNUM                     |; save new user number
    |;debugPrintStrI  "\r\nU:"
    |;debugPrintHexByte %d0
    |;debugPrintHexNyb    %d0
    move.l  PROCNUM,%d0
...
RestoreUserContext:
    ...
    lea     PROCTABLE,%a0                   |; get user table pointer again
...
doSysTrapConRead:
    ...
    lea     PROCTABLE,%a0                   |; get pointer to user table
    move.l  PROCNUM,%d0                     |; get user number
    mulu    #utbl_size,%d0                  |; shift to offset in table
    add.l   %d0,%a0                         |; pointer to user table entry
    move.l  %a0@(utblConInDrv),%a1          |; get pointer to console driver
    move.l  %a0@(utblConInDev),%a0          |; get pointer to console device
    pea     %pc@(1f)                        |; push return address
    jmp     %a1@                            |; jump to driver routine
1:
    movem.l %sp@+,%a0-%a1                   |; restore working registers
    rte

doSysTrapConWrite:
    movem.l %a0-%a1/%d2,%sp@-               |; save working registers
    lea     PROCTABLE,%a0                   |; get pointer to user table
    move.l  PROCNUM,%d2                     |; get user number
    mulu    #utbl_size,%d2                  |; shift to offset in table
    add.l   %d2,%a0                         |; pointer to user table entry
    move.l  %a0@(utblConOutDrv),%a1         |; get pointer to console driver
    move.l  %a0@(utblConOutDev),%a0         |; get pointer to console device
    pea     %pc@(1f)                        |; push return address
    jmp     %a1@                            |; jump to driver routine
1:
    bcc.s   2f                              |; branch if tx not ready
    movem.l %sp@+,%a0-%a1/%d2               |; restore working registers
    rte                                     |; we're done here
2:                                          |; Tx not ready
    movem.l %sp@+,%a0-%a1/%d2               |; restore working registers
